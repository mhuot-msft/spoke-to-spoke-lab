...
// FIXED: Adjacent PEs — place DFS + Blob PEs in the VM's VNet so traffic
// stays local and bypasses the forced-tunneling UDR entirely.
module adjacentPe 'modules/storage/adjacent-pe.bicep' = {
  name: 'adjacentPe'
  params: {
    location: location
    tags: tags
    storageAccountId: adlsAccount.outputs.storageAccountId
    subnetId: spokeDbrxVnet.outputs.peSubnetId
    dfsZoneId: privateDnsZones.outputs.dfsZoneId
    blobZoneId: privateDnsZones.outputs.blobZoneId
  }
}
